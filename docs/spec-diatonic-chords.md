...
- 各コード列に **再生領域（セル枠全体）** と **選択** ボタンを設ける
- **選択** → **コードルート / コード種別の選択を更新**し、指板・構成音パネルと連動させる（再生はしない）
- **再生** → セル枠全体が再生領域（▶ は表示のみ）。**選択** ボタン押下時は再生しない
...
各列（コード 1 個）の下部に **3 ボタン**（▶ 表示・∞ リピート・選択）を配置する:

| ボタン | ラベル | 動作 |
|--------|--------|------|
| 再生 | セル枠全体 | セル枠タップで **`chordId` あり** → `playChord`、**なし** → `playChordSemitonesFromRoot`（▶ は表示のみ）。**∞** / **選択** 押下時は再生しない |
| リピート | `∞`（再生中 `■`） | **`chordId` あり** → `playChordRepeat`、**なし** → `playChordRepeatSemitonesFromRoot`。4/4・4 分音符 4 拍を **■ 停止までループ** |
...
| 1.11 | 2026-05-20 | セル枠全体を再生領域化（▶ は表示のみ） |
| 1.12 | 2026-05-30 | 各列に ∞ リピートボタン追加（4/4 ループ、再生中 ■ 停止） |
